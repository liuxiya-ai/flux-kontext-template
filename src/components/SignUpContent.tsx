...
export function SignUpContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : t('failureMessage'))
    } finally {
      setIsLoading(false)
    }
  }
          ...
          {(process.env.NEXT_PUBLIC_AUTH_GOOGLE_ENABLED === "true" || process.env.NEXT_PUBLIC_AUTH_GITHUB_ENABLED === "true") && (
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-background text-muted-foreground">{t('withEmail')}</span>
              </div>
            </div>
          )}
